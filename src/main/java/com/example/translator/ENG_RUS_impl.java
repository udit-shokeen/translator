...
@Slf4j
public class ENG_RUS_impl implements Translator{
    @Override
    public TranslatorResponse translate(TranslatorRequest request, Database database, LLMModel model) {
        ...
        List<String> translatedTokens = sentences.stream().map(sentence -> {
            String key = TranslatorType.ENG_RUS.name() + "__" + sentence;    // Prefixing with translator type for key creation
            Optional<String> translation = database.getTranslation(key);
            if (translation.isPresent()) {
                return translation.get();
            } else {
                LLMModelResponse translated = model.getTranslationFromModel(sentence, Language.ENG, Language.RUS);
