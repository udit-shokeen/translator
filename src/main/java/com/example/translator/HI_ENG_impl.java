...
@Slf4j
public class HI_ENG_impl implements Translator{
    @Override
    public TranslatorResponse translate(TranslatorRequest request, Database database, LLMModel model) {
        ...
        List<String> translatedTokens = sentences.stream().map(sentence -> {
            String key = TranslatorType.HI_ENG.name() + "__" + sentence;    // Prefixing with translator type for key creation
            Optional<String> translation = database.getTranslation(key);
            if (translation.isPresent()) {
                return translation.get();
            } else {
                LLMModelResponse translated = model.getTranslationFromModel(sentence, Language.HI, Language.ENG);
